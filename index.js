...
import { siGithub } from "simple-icons"; // Brand icons https://simpleicons.org/
...
Alpine.store("portfolio", {
  ...
  async loadPages() {
    this._fetchPromise = (async () => {
      const res = await fetch("/pages/projects.json", {
        cache: "no-cache",
      });
  ...
  toggleFilter(tag) {
    const i = this.filter.indexOf(tag);
    if (i === -1) this.filter.push(tag);
  ...
  get filteredPages() {
    if (this.filter.length === 0) return this.pages;
    const pages = this.pages.filter((p) =>
      p.tagsList?.some((l) => this.filter.includes(l))
    );
    return pages;
  },
});
